...
class HomeproductViewModel : BaseObservable() {
    var onservse = ObservableField<ProductsDatum>()
    var catModel : ProductsDatum? = null
    var context : Fragment? = null
    fun setdata(catModel: ProductsDatum , context : Fragment) {
        onservse.set(catModel)
        this.catModel = catModel
        this.context = context
    }
    fun clickitem(){
        var intent: Intent = Intent(context?.requireActivity(), DetailsproductsActivity::class.java)
