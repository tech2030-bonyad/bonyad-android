...
class UserhomeViewModel(var catogaryFragment: UserhomeActivity) : BaseObservable() {
    var activity: UserhomeActivity = UserhomeActivity()
    var fragment: UserhomeFragment = UserhomeFragment()
...
//        if (LoginSession.isLogin){
    ...
    fun appointment() {
        if (type.get()!= 1){
            if (LoginSession.isLogin){
                var fragment = UserappointmentFragment()
                replaceFragment(fragment)
                type.set(1)
            }else {
