...
import fudex.bonyad.ui.Fragment.merchant.MerchantprofileFragment
import fudex.bonyad.ui.Fragment.technical.TechnicalappointmentFragment
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class MerchanthomeViewModel(var catogaryFragment: MerchanthomeActivity) : BaseObservable() {
    var activity: MerchanthomeActivity = MerchanthomeActivity()
    var fragment: MerchanthomeFragment = MerchanthomeFragment()
...
//        if (LoginSession.isLogin){
    ...
    fun appointment() {
        if (type.get()!= 1){
            if (LoginSession.isLogin){
                var fragment = MerchantordersFragment()
                replaceFragment(fragment)
                type.set(1)
            }else {
    ...
    fun proofile() {
        if (type.get()!= 3) {
            var fragment = MerchantprofileFragment()
            replaceFragment(fragment)
            type.set(3)
        }
    }
