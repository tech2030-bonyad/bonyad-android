...
class EdittradeViewModel(activity: EdittradeActivity) : BaseObservable() {
    var activity: EdittradeActivity = EdittradeActivity()
    ...
    fun editparofile(){
            ...
            if (LoginSession.gettype(activity) == 1){
                requestBody =  MultipartBody.Builder()
                    ...
                    .addFormDataPart("name", userdata.get()?.data?.name ?: "")
                    .addFormDataPart("email", userdata.get()?.data?.email ?: "")
                    .build()
            }else if (LoginSession.gettype(activity) == 2){
                requestBody =  MultipartBody.Builder()
                    .setType(MultipartBody.FORM)
                    .addFormDataPart("name", userdata.get()?.data?.name ?: "")
                    .addFormDataPart("address", addressObserv.get().toString())
                    .addFormDataPart("trade_name", tradenameObserv.get().toString())
                    .addFormDataPart("email", userdata.get()?.data?.email ?: "")
                    .addFormDataPart("description_of_business_activity", desObserv.get().toString())
                    .build()
            }
    ...
    fun editprofilewithimage(){
        ...
        val name = RequestBody.create(MediaType.parse("text/plain"), userdata.get()?.data?.name ?: "")
        val email = RequestBody.create(MediaType.parse("text/plain"), userdata.get()?.data?.email ?: "")
        val adress = RequestBody.create(MediaType.parse("text/plain"), addressObserv.get().toString())
        val tradename = RequestBody.create(MediaType.parse("text/plain"), tradenameObserv.get().toString())
        val des = RequestBody.create(MediaType.parse("text/plain"), desObserv.get().toString())

        val call: Call<ProfileModel?>? = apiService.editprofilewithimage(imageBody,name,email, if (LoginSession.gettype(activity) == 1){null}else{adress}, if (LoginSession.gettype(activity) == 1){null}else{tradename}, if (LoginSession.gettype(activity) == 1){null}else{des})
        call?.enqueue(object : Callback<ProfileModel?> {
            override fun onResponse(call: Call<ProfileModel?>, response: Response<ProfileModel?>) {
                if (response.code() == 200 || response.code() == 201) {
        ...
        call?.enqueue(object : Callback<ProfileModel?> {
            override fun onResponse(call: Call<ProfileModel?>, response: Response<ProfileModel?>) {
                if (response.code() == 200 || response.code() == 201) {
                    ...
                    img.set(data?.data?.business_logo!!)
                    userdata.set(data)
                    addressObserv.set(data?.data?.address!!)
                    tradenameObserv.set(data?.data?.trade_name!!)
                    desObserv.set(data?.data?.description_of_business_activity!!)
                    notifyChange()
                }else {
                    val errorText = response.errorBody()?.string()
                    val errorResponse = Gson().fromJson(errorText, ErrorResponse::class.java)
